...
void eval_int_euler(cbd_block_t * block, sim_state_t * state){
    if(!block->cache){
        ...
        cbd_param_t * p_0 = d_array_at(&state->cbd_signals, initial);
        cache->initial = d_array_at(&state->values, p_0->value);

        cbd_param_t * p_1 = d_array_at(&state->cbd_signals, timestep);
        cache->initial = d_array_at(&state->values, p_1->value);
...
int cbd_block_standard_int_euler(int in, int out, int initial, const char * name, sim_state_t * state){
    ...
    int block = cbd_block_add(name, pin, 2, pout, 2, params, 2, eval_int_euler, 1, state);
