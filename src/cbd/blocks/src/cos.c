...
void eval_cos(cbd_block_t * block, sim_state_t * state){
    if(!block->cache){
        ...
        eval_state_cos_t * cache = malloc(sizeof(eval_state_cos_t));
    ...
    eval_state_cos_t * c = (eval_state_cos_t*)block->cache;
    *c->out = (*c->A) * cos(2*M_PI* (*c->f) * (*c->time));
}

int cbd_block_src_cos(int out, int A, int f, const char * name, sim_state_t * state){
    const int pin[1] = {state->time};
    const int pout[1] = {out};
    const int params[2] = {A, f};
    int block = cbd_block_add(name, pin, 1, pout, 1, params, 2, SRC_COS, 0, state);
