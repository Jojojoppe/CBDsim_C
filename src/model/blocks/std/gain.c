...
void std_gain_generate(FILE * f, block_t * block){
    ...
    fprintf(f, "signals[%d] = params[%d]*signals[%d];\n", out, pk, in);
}

void std_gain_generate_init(FILE * f, block_t * block){
}
...
int blocks_add_std_gain(double k, const char * name, int in, int out, model_t * model){
    ...
    return model_add_block(pin, 1, pout, 1, params, 1, name, 
        "std/gain", 0, std_gain_generate, std_gain_generate_init, model
    );
}