...
int main(int argc, char ** argv){
    ...
    sim_load("test.model", 0.01, &state);
    sim_compile(&state);

    int signal_e1 = sim_get_signal("e1", &state);
    int signal_e2 = sim_get_signal("e2", &state);
    int signal_e3 = sim_get_signal("e3", &state);
    int signal_f = sim_get_signal("f", &state);
